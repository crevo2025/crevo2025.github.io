...
export const BookingCalendar: React.FC<BookingCalendarProps> = ({ onDateSelect, isAdmin = false }) => {
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-[var(--bg-surface)] rounded-sm border border-[var(--border-color)] overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-color)]">
        <h2 className="text-lg font-light tracking-[0.2em] text-[var(--text-primary)]">
          {format(currentMonth, 'yyyy年 M月', { locale: ja })}
        </h2>
        <div className="flex gap-2">
          <button 
            onClick={prevMonth}
            className="p-2 hover:bg-[var(--bg-dark)] rounded-full transition-colors text-[var(--text-muted)] hover:text-[var(--text-primary)]"
          >
            <ChevronLeft size={18} />
          </button>
          <button 
        ...
        {weekDays.map((day, index) => (
          <div 
            ...
            className={`py-3 text-center text-[10px] font-medium tracking-widest uppercase ${
              index === 0 ? 'text-red-400/60' : index === 6 ? 'text-blue-400/60' : 'text-[var(--text-muted)]'
            }`}
          >
            {day}
          </div>
          ...
          return (
            <button
              ...
              className={`
                ...
                ${isSelected ? 'bg-[#c08457] text-white' : 'hover:bg-[var(--bg-dark)] text-[var(--text-secondary)]'}
      ...
      {/* Legend */}
      <div className="px-6 py-4 bg-[var(--bg-dark)] flex justify-center gap-6 text-[10px] tracking-widest text-[var(--text-muted)] uppercase">
